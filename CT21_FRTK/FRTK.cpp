// CT21FRTK.cpp: implementation of the CCT21FRTK class.
//
//////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////
void CFRTKface::load(const char* str)
{
	int num;
	CvPoint p1, p2;
...
void CFRTKface::save(const char* str)
{
	CvPoint p1, p2;
...
void CFRTKface::savefeature(const char* str)
{
	FILE *fp;
...
void CFRTKface::draw(IPLIMAGE src)
{
...
void CFRTK::FRTKinit(IPLIMAGE img)
{
	if (init_flag)	return;
	...
	for (i=0; i<LPsize.width; i++)
		for (j=0; j<LPsize.height; j++)
		{
			...
			((uchar*)(xmap->imageData + xmap->widthStep*j))[i] = x;
			((uchar*)(ymap->imageData + ymap->widthStep*j))[i] = y;
		}
...
void CFRTK::FRTKcolorconvert(IPLIMAGE src, IPLIMAGE dst, CString str)
{
	/*
	int i;
	/*
...
void CFRTK::FRTKfiltering(IPLIMAGE src, IPLIMAGE dst, CString str, int minmax[2])
{
	int i;
	imgview->SetImagePlane(1, 1);
	for (i=0; i<4; i++)
	...
	if (str.Find("MEDIAN") == -1)
	{
		...
		else if (!str.Compare("GABOR"))
		{
			...
			FRTKnormalize(floatimg[1], grayimg[1]);
		}
...
// 20030420
// FT IFT DCT IDCT
void CFRTK::FRTKfourier(IPLIMAGE src, IPLIMAGE dst, CString str)
{
	int i;
	imgview->SetImagePlane(1, 1);
	for (i=0; i<4; i++)
...
//20030417
//LABELING, MAX_BLOB
void CFRTK::FRTKblobanal(IPLIMAGE src, IPLIMAGE dst, CString str, CString morph, int num)
{
	int i;	
	/*
	imgview->SetImagePlane(1, 1);
...
void CFRTK::FRTKcoordtx(IPLIMAGE src, IPLIMAGE dst, CString str)
{
	int i;
	...
	for (i=0; i<LPsize.width; i++)
		for (j=0; j<LPsize.height; j++)
		{
			...
			((uchar*)(xmap2->imageData + xmap2->widthStep*j))[i] = x;
			((uchar*)(ymap2->imageData + ymap2->widthStep*j))[i] = y;
		}*/
...
//void CFRTK::FRTKhistogram(IPLIMAGE Simg, IPLIMAGE Dimg, )

//20030420
void CFRTK::FRTKproject2(IPLIMAGE Simg, CvMat proj, int dir, int method)
{
...
//20030417
//20030420
void CFRTK::FRTKprojext(IPLIMAGE src, IPLIMAGE dst, CString str)
{
	int i;
	...
	{
		FRTKproject(grayimg[0], grayimg[2], 1, 0, 0);
	}
	else if (!str.Compare("VERT"))
	{
		FRTKproject(grayimg[0], grayimg[1], 0, 1, 0);
	}
	...
	{
		FRTKproject2(grayimg[0], projY, 1);
		FRTKcompext(grayimg[0], projY, cvPoint(0, grayimg[0]->height), face);
...
void CFRTK::FRTKthreshold(IPLIMAGE src, IPLIMAGE dst, CString str, int th1, int th2)
{
	int i;
	...
	else if (!str.Compare("HIST"))
	{
		if (src->nChannels == 3)
		{
			...
			FRTKhistogram(grayimg[4], grayimg[1], 0);
			FRTKhistogram(grayimg[5], grayimg[2], 0);
			FRTKhistogram(grayimg[6], grayimg[3], 0);

			debug[1]->ShowSingleImage(grayimg[2]);
			debug[2]->ShowSingleImage(grayimg[3]);
		}
		
		FRTKhistogram(grayimg[0], grayimg[1], 0);
	}
...
//20030420
void CFRTK::FRTKproject(IplImage *Simg, IplImage *Dimg, int xdir, int ydir, int method)
{
	...
	// get projection
	if (xdir) FRTKproject2(Simg, projX, 0, method);
	if (ydir) FRTKproject2(Simg, projY, 1, method);
...
//20030420
void CFRTK::FRTKhistogram(IplImage *Simg, IplImage *Dimg, int start)
{
	int val, i, MaxVal = 0;
	int bins = 256;
...
void CFRTK::FRTKtempmatch(IPLIMAGE src, IPLIMAGE dst, CString str, CString part, int degree)
{
	int i, j;
	/*
	imgview->SetImagePlane(1, 1);
...
//	if (pDlg->DoModal())
	{
		...
		//if (filename != "")
		{
			IPLIMAGE cimg = cvvLoadImage("k:\\face_template.jpg");
...
void CFRTK::FRTKstatfeat(IPLIMAGE src, IPLIMAGE dst, CString str)
{
	CString str2;
	int i;
	imgview->SetImagePlane(1, 1);
...
void CFRTK::FRTKgeofeat(IPLIMAGE src, IPLIMAGE dst, CString str, CString str2, CFRTKface face)
{
	CString str3;
	int i;
	imgview->SetImagePlane(1, 1);
	...
	{		
		...
		FRTKnormalize(floatimg[i+1], grayimg[i+6]);
	}
	...
	if (!str2.Compare("NONE"))
	{
		...
		{
			FRTKsubfeatext(grayimg[5], &(face.left_eye), EYEOPENLP);
			FRTKsubfeatext(grayimg[5], &(face.right_eye), EYEOPENLP);
			
			face.EyeBrowFeat();
			
			//FRTKsubfeatext(grayimg[6], &(face.mouth), CNTNZ);
			FRTKsubfeatext(grayimg[8], &(face.mouth), GG);
			
			FRTKsubfeatext(grayimg[6], &(face.nasolabial_root), NLR);
			
			FRTKsubfeatext(grayimg[7], &(face.left_nasolabial_fold), NLF);
			FRTKsubfeatext(grayimg[7], &(face.right_nasolabial_fold), NLF);
...
/*
void CFRTK::FRTKfacefeatext(IPLIMAGE Simg, CFRTKface *face)
{
	static int k = 0;
	int i;
	...
	FRTKsubfeatext(grayimg[5], &(face->left_eye), EYEOPENLP);		// LP
	FRTKsubfeatext(grayimg[5], &(face->right_eye), EYEOPENLP);		// LP
	//FRTKsubfeatext(grayimg[8], &(face->left_eye), EYEOPENG);	// gabor
	//FRTKsubfeatext(grayimg[8], &(face->right_eye), EYEOPENG);	// gabor
	FRTKsubfeatext(grayimg[6], &(face->nasolabial_root), NLR);
	FRTKsubfeatext(grayimg[7], &(face->left_nasolabial_fold), NLF);
	FRTKsubfeatext(grayimg[9], &(face->right_nasolabial_fold), NLF);
	FRTKsubfeatext(grayimg[6], &(face->mouth), CNTNZ);
...
// 20030502 -> GG 추가
void CFRTK::FRTKsubfeatext(IPLIMAGE Simg, CFRTKsubcomp *comp, FeatureIndex method)
{
	int th = 0;
	int i, j;
	...
	switch(method)
	{
		...
		case NLF:
			...
			iplSet(grayimg[1], 0);
			FRTKthreshold2(grayimg[0], grayimg[1], th, 0, OTSU_H, 0.8f);
		...
		case NLR:
			...
			iplSet(grayimg[1], 0);
			FRTKthreshold2(grayimg[0], grayimg[1], th, 0, OTSU_H, 0.5);
		...
		case CNTNZ:
			// 0: get bin. image (GT)
			iplSet(grayimg[1], 0);
			FRTKthreshold2(grayimg[0], grayimg[1], 64, 0, GT, 0.5f);
		...
		case EYEOPENLP:
			...
			iplSet(grayimg[1], 0);
			FRTKthreshold2(grayimg[0], grayimg[1], th, 0, OTSU_H, 1.8f);
			...
			FRTKproject2(grayLPimg[0], projXLP, 0);
		...
		case EYEOPENG:
			...
			// get bin. image (GT) by otsu's method with 0.5
			iplSet(grayimg[1], 0);
			FRTKthreshold2(grayimg[0], grayimg[1], th, 0, OTSU_H, 0.5f);
...
void CFRTK::FRTKnormalize(IplImage *Simg, IplImage *Dimg)
{
	float min, max, scale;
...
void CFRTK::FRTKthreshold2(IPLIMAGE Simg, IPLIMAGE Dimg, int low, int high, ThresholdIndex method, float otsu)
{
	int th = 0;
...
void CFRTK::FRTKotsu(IplImage *Simg, int *th0, int start)
{
	int i;
	int bins = 256;
...
// 20030502
// -_-++++++++++++++++++++++++++
void CFRTK::FRTKcompext(IPLIMAGE A, CvMat proj, CvPoint range, CFRTKface& facecomp)
{
	int i, j;
	...
	CFRTKsegment* segment = NULL;
	segment = new CFRTKsegment[est_num+1];
	...
	if (facecomp.left_eyebrow.rect.height > 0)
	{
		...
		FRTKproject2(grayimg[1], projX, 0);
		FRTKsubsearch(projX, cvPoint(0, A->width), 0, facecomp);
	}
	...
	if (facecomp.left_eye.rect.height > 0)
	{
		...
		FRTKproject2(grayimg[1], projX, 0);
		FRTKsubsearch(projX, cvPoint(0, A->width), 1, facecomp);
	}
	...
	if (facecomp.nose.rect.height > 0)
	{
		...
		FRTKproject2(grayimg[1], projX, 0);
		FRTKsubsearch(projX, cvPoint(0, A->width), 2, facecomp);
	}
	...
	if (facecomp.mouth.rect.height > 0)
	{
		...
		FRTKproject2(grayimg[1], projX, 0);
		FRTKsubsearch(projX, cvPoint(0, A->width), 3, facecomp);
	}
...
// eyes and eyebrows.
// even mouth and nose.
void CFRTK::FRTKsubsearch(CvMat proj, CvPoint range, int mode, CFRTKface& facecomp)
{
	int i, j;
	int est_num = 0, est_num2 = 0;
	CFRTKsegment *segment;
...
void CFRTK::FRTKalu(IPLIMAGE src, IPLIMAGE dst, CString str, BOOL flag[3], float Const)
{
	int i;
	...
	if (!str.Compare("AND"))
	{
		iplAnd((main->GetMemoryImage(1))->GetSingleImage(), (main->GetMemoryImage(2))->GetSingleImage(), grayimg[4]);
	}
	else if (!str.Compare("OR"))
	{
		iplOr((main->GetMemoryImage(1))->GetSingleImage(), (main->GetMemoryImage(2))->GetSingleImage(), grayimg[4]);
	}
	else if (!str.Compare("NOT"))
	{
		iplNot((main->GetMemoryImage(1))->GetSingleImage(), grayimg[4]);
	}	
	else if (!str.Compare("XOR"))	
	{
		iplXor((main->GetMemoryImage(1))->GetSingleImage(), (main->GetMemoryImage(2))->GetSingleImage(), grayimg[4]);
	}
	else if (!str.Compare("MULT"))
	{
		iplMultiply((main->GetMemoryImage(1))->GetSingleImage(), (main->GetMemoryImage(2))->GetSingleImage(), grayimg[4]);
	}
	else if (!str.Compare("DIV"))
	{
		//iplDivide((main->GetMemoryImage(1))->GetSingleImage(), (main->GetMemoryImage(2))->GetSingleImage(), grayimg[4]);
	}
	else if (!str.Compare("SUB"))
	{
		iplSubtract((main->GetMemoryImage(1))->GetSingleImage(), (main->GetMemoryImage(2))->GetSingleImage(), grayimg[4]);
	}
	else if (!str.Compare("ADD"))
	{
		iplAdd((main->GetMemoryImage(1))->GetSingleImage(), (main->GetMemoryImage(2))->GetSingleImage(), grayimg[4]);
	}
...
// 20030507
// MainFrm 과의 통신 문제??
void CFRTK::FRTKrecFMMNN(CString str, float param[])
{
	int i, j;
	float **data;
	int *classInfo;
...
//20030507
void CFRTK::FRTKrecNN(CString str, float param[])
{
	CMainFrame* mainframe = (CMainFrame*)AfxGetMainWnd();
...
// free, save, load...
void CFRTK::FRTKrecFNN(CString str, float param[])
{
	CMainFrame* mainframe = (CMainFrame*)AfxGetMainWnd();
...
//20030508
// 삽질 성공 -_-v~
void CFRTK::FRTKeigenobject(IPLIMAGE src, IPLIMAGE dst, CString str, float param[])
{
	int i;
	...
	if (!str.Compare("TRAIN"))
	{
		if (pDlg->DoModal())
		{
			...
			// show results
			FRTKnormalize(output[0], avgimg2);
			debug[0]->ShowSingleImage(avgimg2);
			FRTKnormalize(output[1], avgimg2);
			debug[1]->ShowSingleImage(avgimg2);
			FRTKnormalize(output[2], avgimg2);
			debug[2]->ShowSingleImage(avgimg2);
			FRTKnormalize(avgimg, avgimg2);
...
//2004.3.11
void CFRTK::FRTKferec(IPLIMAGE src, IPLIMAGE dst, CString str, float param[])
{
	register int i;
	...
	FRTKcolorconvert(src, colimg[0], "RGB2HLS");

	//debug[2]->ShowSingleImage(colimg[0]);
	
	cvCvtPixToPlane(colimg[0], grayimg[7], grayimg[8], grayimg[9], NULL);

	//debug[2]->ShowSingleImage(grayimg[0]);
	
	FRTKthreshold2(grayimg[7], grayimg[3], 0, 40, IB, 0.0f);

	FRTKblobanal(grayimg[3], grayimg[5], "MAX_BLOB", "");

	debug[0]->ShowSingleImage(grayimg[5]);

	FRTKthreshold2(grayimg[8], grayimg[4], 0, 100, IB, 0.0f);

	debug[1]->ShowSingleImage(grayimg[4]);
}