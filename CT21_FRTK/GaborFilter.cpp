...
void GaborFilter::gaborFiltering(IPLIMAGE img, IPLIMAGE dst, float wave, float theta)
{
//	int i, j;

	Filter_mask real_part, img_part;    
	IplConvKernelFP	*kernel[1][2];
	IPLIMAGE floatimg[2];

	making_gabor_filter3((float)wave, 1.5f, (float)M_PI*((45.0f)/180.0f), (float)M_PI*(((float)theta)/180.0f), &real_part, &img_part);

	kernel[0][0] = iplCreateConvKernelFP(real_part.hsize, real_part.vsize, real_part.hsize/2, real_part.vsize/2, (float *)real_part.co);
	kernel[0][1] = iplCreateConvKernelFP(img_part.hsize, img_part.vsize, img_part.hsize/2, img_part.vsize/2, (float *)img_part.co);

	floatimg[0] = cvCreateImage(cvSize(img->width, img->height), IPL_DEPTH_32F, 1);

	iplScaleFP(img, floatimg[0], 0.0f, 255.0f);	
	iplConvolve2DFP(floatimg[0], dst, kernel[0], 2, IPL_SUMSQROOT);

	iplDeleteConvKernelFP(kernel[0][1]);
	iplDeleteConvKernelFP(kernel[0][0]);

	delete [] real_part.co;
	delete [] img_part.co;

	cvReleaseImage(&(floatimg[0]));
}

double *GaborFilter::gaborCo(IPLIMAGE img, double wave, double theta)
{
	int i, j;
	...
	floatimg[0] = cvCreateImage(cvGetSize(img), IPL_DEPTH_32F, 1);
	floatimg[1] = cvCreateImage(cvGetSize(img), IPL_DEPTH_32F, 1);

	iplScaleFP(img, floatimg[0], 0.0f, 255.0f);	
	iplConvolve2DFP(floatimg[0], floatimg[1], kernel[0], 2, IPL_SUMSQROOT);

	double *gabor_co = new double[img->width*img->height];
	//float pixel;
	...
	for (j=0; j<img->height; j++)
		...
		{
			gabor_co[i + img->width*j] = ((uchar*)(floatimg[1]->imageData + floatimg[1]->widthStep*j))[i];
