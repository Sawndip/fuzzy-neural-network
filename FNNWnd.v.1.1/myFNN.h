...
public:
	...
	void learnFNN2(float *input, float *output, int no_sample, int *inputs, int no_input);	// 20030902 NRPAL included
	
	float learnFNN_single(int epoch);
	void learnFNN_Thread();
	...
	void ioHistogram();
	void setMembership();
	void recTrain(float *clr);
	void recTest(float *clr);
	...
	void DataTransform(int* class_names, int no_class);

	CImageBaseView*	imgview;
	CLogView* logview;
	CMyChart* chartview[2];
	CMyScrollChart* scrollchartview;
