...
UINT ThreadProc_LearningFNN(LPVOID lparam)
{
	myFNN* myFNN2 = (myFNN*)lparam;
	...
	while((!myFNN2->break_flag))
	{
		errors = myFNN2->learnFNN_single(epoch);

		if (errors < myFNN2->err_criteria)
			break;

		epoch++;

		if (epoch > myFNN2->epoch)
			break;
	}

	message.Format("Learning Stop: #%dth trial [MSE %.3f]", epoch, errors);
	myFNN2->logview->AddLine(message);

	myFNN2->recTrain(&clr);
	myFNN2->recTest(&clr);

	myFNN2->break_flag = FALSE;
...
void myFNN::learnFNN(float *input, float *output, int no_sample)
{
	...
	AfxBeginThread(ThreadProc_LearningFNN, (LPVOID)this);

	/*
	CMainFrame* mainframe = (CMainFrame*)lparam;
	CLogView* logview = (CLogView*)(mainframe->GetLogView());
	*/
	...
	for (t=0; t<epoch; t++)
	{
		...
		if (t%10 == 0)
		{
			...
			scrollchartview->mySChart.AddData(1, (float)errors);
			scrollchartview->mySChart.AddData(2, (float)err_criteria);
		}
		...
		{
			message.Format("\r%dth Trial [MSE %.0e]", t, errors);
			logview->AddString(message);
		}
		*/
...
void myFNN::ioHistogram()
{
	int j,k,l;
	float sum;
	float *F_ijml = new float[layer[1]*layer[3]];
	float *G_ij = new float[layer[1]];
	float *H_ijm = new float[layer[1]*layer[4]];
	float fSFvalue[2];

	//CHECK_TIME_START;
	for (j=0; j<layer[1]*layer[4]; j++)		H_ijm[j] = 0.0f;
	for (j=0; j<layer[1]*layer[3]; j++)		F_ijml[j] = 0.0f;
	for (j=0; j<layer[1]; j++)		G_ij[j] = 0.0f;
	...
	for (k=0; k<layer[2]; k++)
	{
		for (j=0; j<layer[1]; j++)
		{
			...
			if (connect[2][k][j] == 1)
			{
				...
				{
					if (connect[3][l][k] == 1)
						//F_ijml[l+j*layer[3]] += fSFvalue[j%2]/(pow(layer[1]/layer[0], layer[0]-1));
						F_ijml[l+j*layer[3]] += 1.0f/(pow(layer[1]/layer[0], layer[0]-1));
				}
			}
		}
	}
	*/
	...
	for (j=0; j<layer[1]; j++)
		...
		{
			for (k=0; k<layer[2]; k++)
				if (connect[2][k][j] && connect[3][l][k])
					F_ijml[l+j*layer[3]] += 1.0f/(pow(layer[1]/layer[0], layer[0]-1));
		}
	...
	*/
	...
	for (k=0; k<layer[4]; k++)
		for (j=0; j<layer[1]; j++)
		{
			sum = 0.0f;
			for (l=0; l<layer[3]; l++)
				sum += F_ijml[l+j*layer[3]]*(2.0*(float)(l%(layer[3]/layer[4]))/(layer[3]/layer[4]-1)-1.0);

			H_ijm[j+k*layer[1]] = fabs(sum);
		}
	...
	chartview[0]->DrawHistogram(G_ij, layer[1], 1);
	chartview[1]->DrawHistogram(H_ijm, layer[1], layer[4]);
...
// 2004.3.25
// 2004.5.22
void myFNN::recTest(float *clr)
{
	if (!e_no)
		return;

	int i, j, k, emotion_count = 0;
	float ratio = 0.0f, ratioc = 0.0f, max;
	int index, maxindex;
	...
	int *rcmat = new int[out_no*out_no];
	for (i=0; i<out_no; i++)
		for (j=0; j<out_no; j++)
			rcmat[i*out_no+j] = 0;

	//int personID[14] = {300, 350, 350, 350, 250, 350, 300, 350, 350, 350, 350, 350, 350, 350};
	//int personID[8] = {350, 350, 350, 350, 350, 350, 350, 350};
	int personID[10] = {350, 350, 350, 350, 350, 350, 350, 350, 350, 350};
	int personIDindex = 0;
	int personIDcindex = 0;
	...
	for (i=0; i<e_no; i++)		
	{
		...
		if (i == (personID[personIDindex]-1+personIDcindex))
		{
			personIDcindex += personID[personIDindex];
			*clr = ratio/(float)personID[personIDindex]*100.0f;
			message.Format("Recognition Rate for Test Set [%.2f]", *clr);
			logview->AddLine(message);
			personIDindex++;
	...
	*clr = ratioc/(float)e_no*100.0f;
	message.Format("Recognition Rate for Test Set [%.2f]", *clr);
	logview->AddLine(message);

	FILE *fp1;

	fp1 = fopen(".\\cmat.dat", "wt");

	for (j=0; j<out_no; j++)
	{
		for (i=0; i<out_no; i++)
			fprintf(fp1, "%d ", rcmat[j*out_no+i]);
		fprintf(fp1, "\n");
	}
	...
	for (i=0; i<out_no; i++)
	{
		message2.Format("%d ", rcmat[i*out_no+i]);
		message += message2;
	}
	logview->AddLine(message);
...
void myFNN::recTrain(float *clr)
{
	...
	message.Format("\r\nRecognition Rate for Training Set [%.2f]", (*clr = ratio/(float)l_no*100.0f));
	logview->AddLine(message);
...
//20030612
void myFNN::DataTransform(int* class_names, int no_class)
{
	int* index = new int[(int)((float)t_no/(float)out_no)*no_class];

	float *xin = new float[t_no*in_no];
	float *xout = new float[t_no*out_no];
	...
	{
		for (j=0; j<in_no; j++)
			xin[i*in_no+j] = tin[i*in_no+j];
		for (j=0; j<out_no; j++)
			xout[i*out_no+j] = tout[i*out_no+j];
	}
	...
	for (i=0; i<t_no; i++)
	{
		...
		{
			if (tout[i*out_no+class_names[k]] == HI)
			{
				index[l++] = i;
				//tout[i*out_no-offset] = -1.0f;
			}
		}
	}
	...
	{
		for (j=0; j<in_no; j++)
			tin[i*in_no+j] = xin[index[i]*in_no+j];
		for (j=0; j<no_class; j++)
			tout[i*no_class+j] = xout[index[i]*out_no+class_names[j]];
	}

	t_no = (int)((float)t_no/(float)out_no)*no_class;
	l_no = (int)((float)l_no/(float)out_no)*no_class;
	e_no = (int)((float)e_no/(float)out_no)*no_class;
...
//20040102
//20040104
float myFNN::learnFNN_single(int t)
{
	int i, j, k;
	int cbias = 0;
	float sum = 0.0f;
	...
	for (int dummy=0; dummy<5; dummy++)
	{
		...
		evalFNN((lin + exam*layer[0]), out_, 0);
		
		errors = 0.0f;
		for (i=0; i<layer[4]; i++)
		{
			error[i] = *(lout + exam*layer[4] + i) - out_[i];
			errors += (float)(0.5*(error[i]*error[i]));
		}
		...
		if (t%10 == 0)
		{
			scrollchartview->mySChart.AddData(1, (float)log10((double)errors));
			scrollchartview->mySChart.AddData(2, (float)log10((double)err_criteria));
		}
