...
void myFNN::learnFNN(float *input, float *output, int no_sample)
{
	...
	message.Format("\r\nLearning Start! Ep[%.0e] & Err[%.0e]", (float)epoch, err_criteria);
	logview->AddLine(message);
	...
	for (t=0; t<epoch; t++)
	{
		...
		{
			message.Format("Learning End! [%d]th Trial & [MSE %.0e]", t, errors);
			logview->AddLine(message);
			break;
		}
...
void myFNN::loadFN(const char* str)
{
	...
	CString str2;
	str2.Format("\r\nStructure File[%s]", str);
	logview->AddLine(str2);
	str2.Format("Lr[%.2f] Ep[%.0e] Err[%.0e]", learn_rate, (float)epoch, err_criteria);
	logview->AddLine(str2);

	CMainFrame* main = (CMainFrame*)AfxGetMainWnd();
	((CCmdFNN*)main->GetControlView())->m_Ep = epoch;
	((CCmdFNN*)main->GetControlView())->m_Lr = learn_rate;
	((CCmdFNN*)main->GetControlView())->m_Err = err_criteria;

	((CCmdFNN*)main->GetControlView())->UpdateData(FALSE);
}
...
void myFNN::loadDataN1(const char* str, bool normalize)
{
	...
	*/
	...
	str2.Format("\r\nData File[%s]", str);
	logview->AddLine(str2);
	str2.Format("In[%d] Out[%d] Total[%d] Train[%d] Test[%d]", in_no, out_no, t_no, l_no, e_no);
	logview->AddLine(str2);
...
void myFNN::recTest()
{
	...
	message.Format("\r\nRec. Rate for Test Set[%.2f]", ratio/(float)e_no*100.0f);
	logview->AddLine(message);
...
void myFNN::recTrain()
{
	...
	message.Format("\r\nRec. Rate for Training Set[%.2f]", ratio/(float)l_no*100.0f);
	logview->AddLine(message);
...
void myFNN::ruleCombination()
{
	...
	message.Format("Rule Combination");
	logview->AddLine(message);
}
			...
			for (y=0; y<layer[0]; y++)
			{
				...
				for (x=0; x<xbin; x++)
				{
					...
					for (z=0; z<itlayer[y]; z++)
					{
						...
						((uchar*)(imgIn->imageData + imgIn->widthStep*yp))[x*3] = pixel[0];
						((uchar*)(imgIn->imageData + imgIn->widthStep*yp))[x*3+1] = pixel[1];
						((uchar*)(imgIn->imageData + imgIn->widthStep*yp))[x*3+2] = pixel[2];
					}
				}
			}
			...
			for (y=0; y<layer[4]; y++)
			{
				...
				for (x=0; x<xbin; x++)
				{
					...
					for (z=0; z<otlayer[y]; z++)
					{
						...
						((uchar*)(imgOut->imageData + imgOut->widthStep*yp))[x*3] = pixel[0];
						((uchar*)(imgOut->imageData + imgOut->widthStep*yp))[x*3+1] = pixel[1];
						((uchar*)(imgOut->imageData + imgOut->widthStep*yp))[x*3+2] = pixel[2];
					}
				}
			}
...
void myFNN::ShowError(int time, float error)
{
	...
	((uchar*)(imgError->imageData + imgError->widthStep*(ybin - (int)(yf*(float)ybin) - 1)))[(int)xf] = pixel;
...
{
	return (float)(s1*sqrt(M_PI) + s2*sqrt(M_PI) - fSMcross(m1, m2, s1, s2));
}
