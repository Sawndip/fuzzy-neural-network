...
void iplFaceStructure::SaveFeatures(const char *str)
{
	...
	fprintf(fp, "%f ", __max(left_eye.feature, right_eye.feature));

	fprintf(fp, "%f ", nasolabial_root.feature);

	fprintf(fp, "%f ", __max(left_nasolabial_fold.feature, right_nasolabial_fold.feature));

	fprintf(fp, "%f ", mouth.feature);

	fprintf(fp, "%f ", __max(left_eyebrow.feature, right_eyebrow.feature));
...
void CIplFace::iplFaceFeatureExtract(IplImage *Simg, iplFaceStructure *face)
{
	...
	*/
	...
	face->SaveFeatures("D:\\User\\DJKim\\Research\\Figure\\PEKman\\CMU070611.dat");
...
void CIplFace::iplThreshold2(IplImage *Simg, IplImage *Dimg, int low, int high, int method, float otsu)
{
	...
	switch(method)
	{
		...
		case 4:
			iplOtsu1(Simg, &th, 0);
			low_th = (int)((float)th*otsu);
			if (low_th > 250)
				low_th = 250;

			cvThreshold(Simg, Dimg, low_th, 255, CV_THRESH_BINARY);
			break;
		case 5:
			iplOtsu1(Simg, &th, 0);
...
void CIplFace::ShowFace(IplImage *Simg, IplImage *Dimg)
{
	...
	FILE *fp, *fp_;
	fp_ = fopen("D:\\User\\DJKim\\Research\\Figure\\PEkman\\Fe_Code.txt", "rt");
	...
	for (i=0; i<149; i++)
	{
		...
		sprintf(str, "D:\\User\\DJKim\\Research\\Figure\\PEkman\\%s.dat", str_);
		fp = fopen(str, "rt");
		...
		*/
		...
		sprintf(str, "D:\\User\\DJKim\\Research\\Figure\\PEkman\\%s.jpg", str_);
		IplImage *file = cvvLoadImage(str);

		// set each facial component from loaded data
		float p1y, p2y;
		fscanf(fp, "%d %f %d %f", &(p1.x), &p1y, &(p2.x), &p2y);
		p1.y = (int)p1y; p2.y = (int)p2y;
...
void CIplFace::ShowSubFace(IplImage *Simg, IplImage *Dimg, CvRect rect, int method)
{
	...
	switch(method)
	{
		...
		case 2:
			...
			//cvAdaptiveThreshold(procCPGimg[0], procCPGimg[1], 255, CV_STDDEV_ADAPTIVE_THRESH, CV_THRESH_BINARY, param);
			//20040215 ¼öÁ¤¿ä¸Á!
			break;
		case 3:
			iplSet(procCPGimg[1], 0);
