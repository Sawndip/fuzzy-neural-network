...
doubleMatrix doubleArray::map2(const double (*f)(const double&)) const {
  double* newX = new double [M*N];
  double* t = newX;
  double* u =  X;
  double* v =  X + N;
...
doubleMatrix doubleArray::map2(const double (*f)(      double )) const {
  double* newX = new double [M*N];
  double* t = newX;
  double* u =  X;
  double* v =  X + N;
...
doubleMatrix doubleMatrix::tanh2(doubleArray a)
{
  double* newX = new double [a.m()*a.n()];
  double* t = newX;
  double* u =  a.x();
...
void doubleMatrix::rndWgt(double wgt)
{
	int i,j;
	for(j=0;j<L;j++)
		for(i=0;i<M;i++)
			X[i+M*j] = wgt*(rand()/RAND_MAX-0.5);
}
