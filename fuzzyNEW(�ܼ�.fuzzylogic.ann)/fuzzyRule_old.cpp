...
{
//	FuzzySet *oldFset = (FuzzySet *)termSet[n];
	FuzzySet *oldFset = (FuzzySet *)termSet.GetAt(termSet.FindIndex(n));
...
void LinguisticVar::graph()
{
	...
	// check num
	int num = getFuzzySet(0)->GetCount();
	for (int j = 0; j < GetCount(); j++) 
		if (num != getFuzzySet(j)->GetCount()) {
...
//	if (fuzzyRule->thenVars.getLength() != numThenVars) return False;
	...
	for (int i = 0; i < numIfVars; i++) {
		//LinguisticVar *lVar = (LinguisticVar *)ifLingVars[i];
		LinguisticVar *lVar = (LinguisticVar *)ifLingVars.GetAt(ifLingVars.FindIndex(i));
		
		for (int j = 0; j < lVar->GetCount() && 
		strcmp((char *)fuzzyRule->ifVars.GetAt(fuzzyRule->ifVars.FindIndex(i)), lVar->getFuzzySet(j)->getName());
		j++) ;
		if (j == lVar->GetCount()) { // error
			printf ("if part : %s mismatch termset: %s\n", 
				lVar->getName(), (char *)fuzzyRule->ifVars[i]);
			error ++;
		}
	}

	for (i = 0; i < numThenVars; i++) {
		LinguisticVar *lVar = (LinguisticVar *)thenLingVars[i];
		for (int j = 0; j < lVar->GetCount() && 
		strcmp((char *)fuzzyRule->thenVars[i], lVar->getFuzzySet(j)->getName());
		j++);
		if (j == lVar->GetCount()) { // error
...
void FuzzyRuleBase::setRule(int l, ...)
{
	...
	for (int i = 0; i < numIfVars; i++) {
		...
		fuzzyRule->ifVars.append(strdup(string));
	}

	for (i = 0; i < numThenVars; i++) {
		string = va_arg(args, char *);
		fuzzyRule->thenVars.append(strdup(string));
	}
	...
	for (i = 0; i < numIfVars; i++) {
		LinguisticVar *lVar = (LinguisticVar *)ifLingVars[i];
		for (int j = 0; j < lVar->GetCount() && 
		strcmp((char *)fuzzyRule->ifVars[i], lVar->getFuzzySet(j)->getName());
		j++) ;
		if (j == lVar->GetCount()) { // error
			printf ("if part of %d-th rule: %s mismatch termset: %s\n", 
				l+1, lVar->getName(), (char *)fuzzyRule->ifVars[i]);
			error ++;
		}
	}
	...
	for (i = 0; i < numThenVars; i++) {
		...
		if (j == lVar->GetCount()) { // error
			printf ("if part of %d-th rule: %s mismatch termset: %s\n", 
				l+1, lVar->getName(), (char *)fuzzyRule->thenVars[i]);
			error ++;
		}
	}
//	if (error) return ;

	fuzzyRules.insert(fuzzyRule, l);
}
...
//void FuzzyRuleBase::setRulev(int l, CPtrList& list)
{
	...
	FuzzyRule	*fuzzyRule = new FuzzyRule;
	for (int i = 0; i < numIfVars; i++) 
		fuzzyRule->ifVars.append(strdup((char *)list[i]));

	for (int j = 0; j < numThenVars; j++) 
		fuzzyRule->thenVars.append(strdup((char *)list[i++]));
...
void FuzzyRuleBase::setLinguisticVar(int l, LinguisticVar *lingVar)
{
	if (l < numIfVars) { 
		...
		ifLingVars.insert(lingVar, l);
	} else if (l < numIfVars + numThenVars) {
		l -= numIfVars;
		if (l < thenLingVars.GetCount()) {
			delete thenLingVars[l];
			thenLingVars.remove(l);
		}
		thenLingVars.insert(lingVar, l);
	} else {
...
{
	if (i < ifLingVars.GetCount()) return (LinguisticVar *)ifLingVars[i];
	else if (i < ifLingVars.GetCount() + thenLingVars.GetCount())
		return (LinguisticVar *)thenLingVars[i-ifLingVars.GetCount()];
	else return (LinguisticVar *)NULL;
}

FuzzyRule *FuzzyRuleBase::getRule(int l)
{
	if (l < fuzzyRules.GetCount()) return (FuzzyRule *)fuzzyRules[l];
...
FuzzySet *FuzzyRuleBase::inference(SbPList &fuzzyInput)
{
	...
	for (int l = 0; l < fuzzyRules.GetCount(); l++) {
		...
		// for l-th Rules
		//
		FuzzyRule *fuzzyRule = (FuzzyRule *)fuzzyRules[l];	
		// input parts
		...
		for (int n = 0; n < fuzzyRule->ifVars.GetCount(); n++) {
			...
			FuzzySet *A = (FuzzySet *)fuzzyInput[n];
			float mx = 0;
			// max min(X, A)
...
void FuzzyRuleBase::printFuzzyRule(int l)
{
	...
	for (int i = 0; i < getIfPartLength(); i++) {
		...
		char *fuzzyStr = (char *)(getRule(l)->ifVars[i]);
	...
	for (i = 0; i < getThenPartLength(); i++) {
		...
		char *fuzzyStr = (char *)getRule(l)->thenVars[i];
		printf ("\"%s\" is \"%s\" ",  varStr, fuzzyStr);
...
void FuzzyRuleBase::printFuzzyRuleBase(int lingOn)
{
	...
	if(DEBUG)
	for (j = 0; j < getLinguisticVar(i)->GetCount(); j++) {
...
int FuzzyRuleBase::parseTermSet(LinguisticVar *lv, FILE *fin, int &n)
{
	...
	// result:
	printf ("%s Termset[%d fuzzysets] stored\n", lv->getName(), index);
	return n;
}
...
*/
	...
	// result:
	printf ("%s variables: %d linguistic vars stored\n", ifthen, l);
	...
	if (DEBUG) {
		for (l = 0; l < numRules; l++) {
			...
			for (i = 0; i < frule->ifVars.GetCount(); i++)
				printf(" %s", (char *)frule->ifVars[i]);
			for (i = 0; i < frule->thenVars.GetCount(); i++)
				printf(" %s", (char *)frule->thenVars[i]);
			puts("");
		}
	}
